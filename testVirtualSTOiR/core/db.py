...
COLUMNS = ["TAG", "Наименование", "Модель", "Производитель", "Дата_поверки", "Статус"]

# Сколько дней до поверки считать "скоро"
WARN_DAYS  = 30   # жёлтый
ALERT_DAYS = 7    # красный

def get_sensor_status_color(date_str: str) -> str:
    """
    Возвращает цвет маркера по дате поверки:
...
class Database:
    ...
    def get_sensor_by_tag(self, tag: str) -> Optional[dict]:
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM sensors WHERE [TAG] = ?", (tag,))
        row = cursor.fetchone()
        return dict(row) if row else None

    def get_all_tags(self) -> list[str]:
        cursor = self.conn.cursor()
        cursor.execute("SELECT [TAG] FROM sensors ORDER BY [TAG]")
