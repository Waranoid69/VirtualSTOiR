...
class SensorInfoDialog(QDialog):
    def __init__(self, sensor_data: dict, parent=None):
        ...
        self.lbl_tag.setText(f"Код параметра (TAG):   {sensor_data.get('Заводской_номер', '—')}")
        self.lbl_description.setText(f"Наименование:   {sensor_data.get('Наименование_параметра', '—')}")
        self.lbl_model_info.setText(
            f"Модель / Производитель:   "
            f"{sensor_data.get('Тип_датчика', '—')} / {sensor_data.get('Производитель', '—')}"
        )
        self.lbl_last_check.setText(f"Последняя поверка:   {sensor_data.get('Дата_последней_поверки', '—')}")
        ...
        self._tag = sensor_data.get("Заводской_номер", "")
